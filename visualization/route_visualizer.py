...
class RouteVisualizer:
    ...
    def __init__(self, output_dir: str = "output/visualizations"):
        ...
        self.logger.info("已配置多瓦片源支持，提高地图显示可靠性")
    ...
    def _create_resilient_map(self, location: List[float], zoom_start: int = 10) -> folium.Map:
        """
        创建具有多瓦片源fallback机制的地图
        解决地图在外部分享时无法显示的问题
        """
        try:
            # 使用第一个瓦片源创建地图
            primary_provider = self.tile_providers[0]
            m = folium.Map(
                location=location,
                zoom_start=zoom_start,
                tiles=None  # 先不设置瓦片，手动添加
            )

            # 添加主要瓦片源
            folium.TileLayer(
                tiles=primary_provider['url'],
                attr=primary_provider['attribution'],
            ...
            # 添加备用瓦片源作为可选图层
            for provider in self.tile_providers[1:]:
                folium.TileLayer(
                    tiles=provider['url'],
            ...
            # 添加地图加载状态提示
            loading_html = '''
            <div id="map-loading-status" style="position: fixed;
                        top: 10px; left: 50%; transform: translateX(-50%);
                        background: rgba(255,255,255,0.9); padding: 8px 15px;
                        border-radius: 5px; border: 1px solid #ccc;
                        z-index: 1000; font-size: 12px;">
                <i class="fa fa-spinner fa-spin"></i> 正在加载地图...
            </div>
            <script>
                setTimeout(function() {
                    var loading = document.getElementById('map-loading-status');
            ...
            self.logger.info(f"创建 resilient 地图，位置: {location}, 使用 {len(self.tile_providers)} 个瓦片源")
            return m

        except Exception as e:
            self.logger.error(f"创建resilient地图失败: {e}")
            # 降级到基础地图
            return folium.Map(location=location, zoom_start=zoom_start, tiles='OpenStreetMap')

    def visualize_single_route(self, route_data: Dict, save_path: Optional[str] = None) -> str:
        """可视化单条路径"""
        try:
            vehicle_id = route_data.get('summary', {}).get('vehicle_id', 'Unknown')

            # 检查路径数据格式：支持route_sequence和itinerary两种格式
            route_sequence = route_data.get('route_sequence')
            itinerary = route_data.get('itinerary')

            if not route_sequence and not itinerary:
                self.logger.warning(f"车辆 {vehicle_id} 没有路径数据 (route_sequence或itinerary)")
                return None

            # 创建具有多瓦片源的resilient地图
            center_lat, center_lon = self._calculate_route_center(route_data)
            m = self._create_resilient_map(location=[center_lat, center_lon], zoom_start=10)
            ...
            depot_info = route_details.get('depot_info', {})
            depot_coords = depot_info.get('coordinates', {'lat': 30.800835, 'lng': 104.139111})
            depot_coordinates = [depot_coords['lat'], depot_coords['lng']]

            folium.Marker(
                depot_coordinates,
                popup=f"配送中心 {depot_info.get('name', 'A网点')}<br>坐标: {depot_coordinates}",
                tooltip="配送中心",
                icon=self.depot_icon
            ).add_to(m)

            # 添加路径点和连线
            route_coordinates = [depot_coordinates]  # 从配送中心开始

            # 处理路径数据
            stops_data = route_sequence if route_sequence else itinerary
            ...
            for i, stop in enumerate(stops_data):
                ...
                if route_sequence:
                    ...
                    distance = stop.get('travel_distance_km', 0)
                else:
                    # 新格式 (itinerary)
                    coords = stop['coordinates']
                    stop_type = 'pickup' if stop.get('action') == '取货' else 'delivery'
                    order_id = stop.get('order_id', f'ORDER_{i+1}')
                    weight = stop.get('weight_change_kg', 0)
                    arrival_time = stop.get('estimated_arrival', 'N/A')
                    distance = stop.get('distance_from_previous_km', 0)

                route_coordinates.append(coords)

                # 确定图标类型
                icon = self.pickup_icon if stop_type == 'pickup' else self.delivery_icon

                # 添加标记
                popup_text = f"""
                <b>站点 {i+1}: {stop_type.upper()}</b><br>
                订单ID: {order_id}<br>
                坐标: ({coords[0]:.6f}, {coords[1]:.6f})<br>
                重量: {weight:.1f}kg<br>
                到达时间: {arrival_time}<br>
                行驶距离: {distance:.2f}km
                """
                ...
                folium.Marker(
                    coords,
                    popup=popup_text,
                    tooltip=f"{stop_type.upper()} - {order_id}",
                    icon=icon
                ).add_to(m)

            # 返回配送中心
            route_coordinates.append(depot_coordinates)
    ...
    def visualize_all_routes(self, routes_data: List[Dict], save_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            m = self._create_resilient_map(location=[center_lat, center_lon], zoom_start=9)
    ...
    def _calculate_route_center(self, route_data: Dict) -> Tuple[float, float]:
        ...
        if route_sequence:
            ...
            # 新格式 (itinerary)
            lats = [stop['coordinates'][0] for stop in route_data['itinerary']]
            lons = [stop['coordinates'][1] for stop in route_data['itinerary']]
        ...
        # 包含配送中心
        lats.append(depot_coords['lat'])
        lons.append(depot_coords['lng'])
...
def test_route_visualizer():
    ...
    def create_enhanced_route_optimization_visualization(self, route_solutions: Dict) -> str:
        ...
        try:
            ...
            # 创建具有多瓦片源的resilient地图
            m = self._create_resilient_map(location=[center_lat, center_lng], zoom_start=10)
    ...
    def create_route_efficiency_heatmap(self, route_solutions: Dict) -> str:
        ...
        try:
            ...
            # 创建具有多瓦片源的热力图
            m = self._create_resilient_map(location=[center_lat, center_lng], zoom_start=11)
    ...
    def create_vehicle_performance_dashboard(self, route_solutions: Dict) -> str:
        ...
        try:
            ...
            # 计算地图中心
            depot_coord = [104.139111, 30.800835]
            m = self._create_resilient_map(location=[depot_coord[1], depot_coord[0]], zoom_start=10)
