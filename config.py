...
# ===== Gurobi求解器配置 =====
GUROBI_CONFIG = {
    # 单货物3DPP优化配置 - ⚡ 优化为1分钟快速求解（从30分钟优化）
    'single_item_3dpp': {
        'time_limit': 60,        # 求解时间限制 (秒) - 1分钟快速求解
        'mip_gap': 0.10,         # MIP gap (10%) - 放宽精度要求换取速度
        'objective': 'maximize_loading_rate'
    },
    # 多车队LTL 3DPP优化配置 - ⚡ 优化为1分钟快速求解（从30分钟优化）
    'multi_truck_ltl': {
        'time_limit': 60,        # 求解时间限制 (秒) - 1分钟快速求解
        'mip_gap': 0.15,         # MIP gap (15%) - 放宽精度要求
        'objective': 'maximize_loading_rate'
    },
    # 通用配置 - ⚡ 增强启发式算法
    ...
    'presolve': 1,             # 启用预处理
    'cuts': 1,                 # 启用切平面
    'heuristics': 0.70,        # 启发式算法时间占比 - 从5%提升到70%
...
ADVANCED_SOLVER_CONFIG = {
    ...
    'max_solve_time_minutes': 1,            # ⚡ 优化为1分钟（从60分钟优化）
    'solution_pool_size': 10,               # 解池大小
    'mip_focus': 1,                         # ⚡ 调整MIP焦点为可行性优先
    'heuristic_focus': 0.8,                 # ⚡ 增强启发式算法焦点（从0.5提升到0.8）
