...
class LogisticsOptimizationSystemEnhanced:
    ...
    def __init__(self, verbose: bool = True, use_enhanced_constraints: bool = True):
        ...
        # 选择优化器版本 - 优先使用快速启发式优化器
        if use_enhanced_constraints:
            # 默认优先使用快速启发式优化器（1分钟求解）
            self.fast_optimizer = None
            self.gurobi_optimizer = None
            self.optimizer_type = "fast_heuristic"  # 默认快速模式

            try:
                from optimization.fast_heuristic_optimizer import FastHeuristicOptimizer
                self.fast_optimizer = FastHeuristicOptimizer()
                if self.verbose:
                    print("[快速] 使用FastHeuristic优化器（1分钟内求解）")
            except ImportError:
                # 如果快速优化器不可用，回退到Gurobi增强
                self.gurobi_optimizer = GurobiOptimizerEnhanced()
                self.optimizer_type = "gurobi_enhanced"
                if self.verbose:
                    print("[增强] 使用Gurobi增强优化器（集成空间-时间约束）")
        else:
            self.gurobi_optimizer = GurobiOptimizerV2()
            self.optimizer_type = "gurobi_v2"
            if self.verbose:
    ...
    def _process_large_orders_enhanced(self, large_orders: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 使用快速启发式优化器进行优化（1分钟内求解）
            if self.optimizer_type == "fast_heuristic" and self.fast_optimizer:
                self.logger.info(f"[快速] 使用FastHeuristic优化大货物")
                # 快速模式：直接调用快速启发式优化器
                fast_result = self.fast_optimizer.solve(
                    items=enhanced_items,
                    truck_specs=truck_specs,
                    max_time_seconds=60  # 1分钟时间限制
                )

                # 转换为标准格式
                enhanced_results = self._convert_fast_result_to_standard_format(fast_result, enhanced_items, "large")
                self.logger.info(f"[完成] 大货物快速优化: {enhanced_results.get('trucks_used', 0)} 辆车")
                return enhanced_results
            else:
                # 回退到Gurobi增强优化器
                optimizer = GurobiOptimizerEnhanced()
                max_items = min(len(enhanced_items), 10)  # 限制复杂度

                self.logger.info(f"🔍 [调试] 开始Gurobi增强优化，最大货物数: {max_items}")
    ...
    def _process_ltl_enhanced(self, medium_orders: pd.DataFrame,
        ...
        try:
            ...
            # 使用快速启发式优化器进行优化（1分钟内求解）
            if self.optimizer_type == "fast_heuristic" and self.fast_optimizer:
                self.logger.info(f"[快速] 使用FastHeuristic优化LTL货物")
                # 快速模式：直接调用快速启发式优化器
                fast_result = self.fast_optimizer.solve(
                    items=all_ltl_items,
                    truck_specs=truck_specs,
                ...
                # 转换为标准格式
                enhanced_results = self._convert_fast_result_to_standard_format(fast_result, all_ltl_items, "ltl")
                self.logger.info(f"[完成] LTL快速优化: {enhanced_results.get('trucks_used', 0)} 辆车")
                return enhanced_results
            else:
                # 回退到Gurobi增强优化器
                optimizer = GurobiOptimizerEnhanced()
                max_items = min(len(all_ltl_items), 15)  # 限制复杂度

                self.logger.info(f"🔍 [调试] 开始LTL Gurobi增强优化，最大货物数: {max_items}")
                result = optimizer.optimize_with_constraints(
                    items=all_ltl_items,
                    truck_specs=truck_specs,
                    max_items=max_items,
    ...
    def _convert_fast_result_to_standard_format(self, fast_result: Dict, enhanced_items: List, item_type: str) -> Dict:
        """将快速启发式优化结果转换为标准格式"""
        try:
            # 检查快速优化结果
            if fast_result is None:
                self.logger.warning("快速优化结果为None，返回空结果")
                return self._empty_large_results() if item_type == "large" else self._empty_ltl_results()

            # 从fast_result中提取数据
            bins = fast_result.get('bins', [])
            trucks_used = fast_result.get('bins_count', 0)
            items_placed = fast_result.get('items_placed', 0)
            loading_rate = fast_result.get('loading_rate', 0.0)
            solve_time = fast_result.get('solve_time_seconds', 0.0)
            ...
            if item_type == "large":
                ...
                for i, bin_obj in enumerate(bins):
                    # 为每个卡车创建一个dispatch结果
                    truck_result = {
                        'truck_id': bin_obj.bin_id,
                        'order_id': f"LARGE_ORDER_{i}",  # 生成订单ID
                        'item_type': item_type,
                        'items_count': len(bin_obj.items),
                        'truck_volume_used': bin_obj.current_volume,
                        'loading_efficiency': bin_obj.volume_utilization,
                        'volume_utilization': bin_obj.volume_utilization,
                        'weight_utilization': bin_obj.weight_utilization,
                        'spatial_positions': [],  # 快速优化不提供空间位置
                        'items': []
                    }

                    # 添加货物信息
                    for item in bin_obj.items:
                        truck_result['items'].append({
                            'item_id': item.item_id,
                            'position': [0.0, 0.0, 0.0],  # 快速优化不计算精确位置
                        ...
                        # 添加到空间位置（简化版）
                        truck_result['spatial_positions'].append({
                            'item_id': item.item_id,
                            'position': [0.0, 0.0, 0.0],
                            'dimensions': [item.length, item.width, item.height]
                        })

                    dispatched_orders.append(truck_result)

                return {
                    'optimization_type': 'fast_heuristic',
                    'status': 'completed',
                    'successfully_dispatched_orders': dispatched_orders,
                    'trucks_used': trucks_used,
                    'truck_volume_used': fast_result.get('volume_placed', 0),
                    'dispatch_efficiency': loading_rate,
                    'dispatch_results': dispatched_orders,
                    'solve_time_seconds': solve_time,
                    'algorithm': fast_result.get('method', 'FFDH_Heuristic')
                }
            else:
                # LTL结果格式 - 确保DataFrame有正确的列
                bin_details = fast_result.get('bin_details', [])

                # 为每个bin添加缺失的字段
                for detail in bin_details:
                    if 'original_order_id' not in detail:
                        detail['original_order_id'] = detail.get('bin_id', 'UNKNOWN')
                    if 'item_type' not in detail:
                        detail['item_type'] = 'ltl'
                    if 'volume_m3' not in detail:
                        detail['volume_m3'] = detail.get('volume', 0.0)
                    if 'item_id' not in detail:
                        # 使用第一个货物ID或生成一个
                        detail['item_id'] = detail.get('item_ids', ['LTL_ITEM'])[0] if detail.get('item_ids') else f"LTL_{detail.get('bin_id', 'UNKNOWN')}"

                return {
                    'status': 'completed',
                    'loaded_items': items_placed,
                    'trucks_used': trucks_used,
                    'total_loading_rate': loading_rate,
                    'loading_plan': pd.DataFrame(bin_details),
                    'solve_time_seconds': solve_time,
                    'algorithm': fast_result.get('method', 'FFDH_Heuristic')
                }

        except Exception as e:
            self.logger.error(f"转换快速优化结果失败: {str(e)}")
            return self._empty_large_results() if item_type == "large" else self._empty_ltl_results()
    ...
    def _collect_enhanced_loading_plans(self, complete_solution: Dict,
        ...
        if ltl_results.get('loading_plan') is not None and not ltl_results['loading_plan'].empty:
            for _, row in ltl_results['loading_plan'].iterrows():
                # 使用bin_id作为truck_id，如果没有则使用item_id
                truck_id = row.get('bin_id', row.get('truck_id', f"LTL_{row.get('item_id', 'UNKNOWN')}"))
                if 'LTL_' not in str(truck_id):
                    truck_id = f"LTL_{truck_id}"
