"""
优化任务管理API
Optimization Task Management API

提供管理员端优化任务的管理功能，包括任务创建、进度监控、结果获取等。
"""
...
# 导入优化引擎
from optimization.main_demo import LogisticsOptimizationDemo
from optimization.main_enhanced import LogisticsOptimizationSystemEnhanced
...
# 初始化演示优化器
demo_optimizer = LogisticsOptimizationDemo()


class OptimizationRequest(BaseModel):
    """优化请求模型"""
    optimization_mode: str = "fast"  # "fast", "enhanced", "demo"
    order_count: int = 50
...
async def run_optimization_task(task_id: str, request: OptimizationRequest):
    ...
    try:
        ...
        # 根据模式选择优化器
        if request.optimization_mode == "demo":
            optimizer = demo_optimizer
            progress_callback = lambda progress: websocket_progress_update(task_id, progress)
            result = await optimizer.run_complete_optimization_demo(
                progress_callback=progress_callback,
                order_count=request.order_count,
                optimization_mode=request.config.get("mode", "fast") if request.config else "fast"
            )
        elif request.optimization_mode == "fast":
            # 使用快速优化器
            optimizer = LogisticsOptimizationSystemEnhanced(verbose=False, use_enhanced_constraints=True)
            result = await run_real_optimization_with_progress(optimizer, task_id, "fast")
        elif request.optimization_mode == "enhanced":
            # 使用增强优化器
            optimizer = LogisticsOptimizationSystemEnhanced(verbose=False, use_enhanced_constraints=True)
            result = await run_real_optimization_with_progress(optimizer, task_id, "enhanced")
        else:
            raise ValueError(f"不支持的优化模式: {request.optimization_mode}")
...
async def run_real_optimization_with_progress(optimizer, task_id: str, mode: str) -> Dict[str, Any]:
    """运行真实优化并更新进度"""
    # 模拟进度更新（真实优化通常不提供实时进度）
    steps = [
        ("系统初始化", 10),
        ("数据预处理", 25),
        ("货物分类", 40),
    ...
    for step_name, progress in steps:
        await update_task_status(task_id, "running", f"正在执行: {step_name}...", progress)
        await asyncio.sleep(1)  # 模拟处理时间

    # 运行实际优化
    print(f"[优化] 开始运行真实优化 ({mode} 模式)")
    ...
    try:
        ...
        formatted_result = _format_real_result(result, mode, optimization_time)

        return formatted_result

    except Exception as e:
        print(f"[错误] 真实优化失败: {str(e)}")
        # 如果真实优化失败，回退到演示结果
        return await _fallback_to_demo_result(mode)


def _format_real_result(result: Dict, mode: str, optimization_time: float) -> Dict:
    """格式化真实优化结果"""
    try:
        return {
            "task_id": str(uuid.uuid4()),
            ...
            "source": "real_optimization",
            "success": True,
            **result  # 包含原始结果数据
        }
    except Exception as e:
        print(f"[错误] 结果格式化失败: {str(e)}")
        return {
            "task_id": str(uuid.uuid4()),
            "optimization_mode": mode,
...
async def _fallback_to_demo_result(mode: str) -> Dict:
    """回退到演示结果"""
    print(f"[回退] 使用演示结果替代")

    # 使用演示优化器生成结果
    demo_result = await demo_optimizer.run_complete_optimization_demo(
        order_count=50,
        optimization_mode=mode
    )

    demo_result["source"] = "demo_fallback"
    demo_result["fallback_reason"] = "Real optimization failed, using demo result"
...
async def update_task_status(task_id: str, status: str, message: str, progress: float = None):
    """更新任务状态"""
